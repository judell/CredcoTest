...
class Hypothesis:
    def __init__(self, 
        ...
        #self.api_url = 'http://%s/api' % self.domain
        ...
        self.max_search_results = 10000 if max_search_results is None else max_search_results  # limit for paginated results
    ...
    def post_annotation(self, payload):
        try:
            ...
            r = requests.post(self.api_url + '/annotations', headers=headers, data=data.encode('utf-8'), timeout=5)
            return r
        except:
    ...
    def update_annotation(self, id, payload):
        headers = {'Authorization': 'Bearer ' + self.token, 'Content-Type': 'application/json;charset=utf-8' }
        data = json.dumps(payload, ensure_ascii=False)
        r = requests.put(self.api_url + '/annotations/' + id, headers=headers, data=data.encode('utf-8'), timeout=5)
        return r
